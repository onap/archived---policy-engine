 ...
 * ============LICENSE_END=========================================================
 */
app.controller('psClosedLoopDictGridController', function ($scope, PolicyAppService, modalService, $modal){
    $( "#dialog" ).hide();
		
    PolicyAppService.getData('getDictionary/get_PSClosedLoopData').then(function (data) {
    	var j = data;
    ...
    PolicyAppService.getData('get_LockDownData').then(function(data){
    	...
    	if($scope.lockdowndata[0].lockdown == true){
    		$scope.psClosedLoopDictionaryGrid.columnDefs[0].visible = false;
    		$scope.gridApi.grid.refresh();
    	}else{
    		$scope.psClosedLoopDictionaryGrid.columnDefs[0].visible = true;
    		$scope.gridApi.grid.refresh();
    	}
    },function(error){
    	console.log("failed");
    ...
    $scope.psClosedLoopDictionaryGrid = {
        ...
        },{ field: 'name', displayName : 'Closed Loop Name', sort: { direction: 'asc', priority: 0 }},
    ...
    $scope.deletePSClosedLoop = function(data) {
        modalService.popupConfirmWin("Confirm","You are about to delete the Closed Loop  "+data.name+". Do you want to continue?",
            function(){
                var uuu = "deleteDictionary/ps_dictionary/remove_PSClosedLoop";
