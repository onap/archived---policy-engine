 ...
 * ============LICENSE_END=========================================================
 */
app.controller('addressGroupDictGridController', function ($scope, PolicyAppService, modalService, $modal){
    $( "#dialog" ).hide();
    
    PolicyAppService.getData('getDictionary/get_AddressGroupData').then(function (data) {
    	var j = data;
    ...
    PolicyAppService.getData('get_LockDownData').then(function(data){
		 ...
		 if($scope.lockdowndata[0].lockdown == true){
			 $scope.addressGroupDictionaryGrid.columnDefs[0].visible = false;
			 $scope.gridApi.grid.refresh();
		 }else{
			 $scope.addressGroupDictionaryGrid.columnDefs[0].visible = true;
			 $scope.gridApi.grid.refresh();
		 }
	 },function(error){
		 console.log("failed");
    ...
    $scope.deleteFWAddressGroup = function(data) {
        modalService.popupConfirmWin("Confirm","You are about to delete the Address Group  "+data.groupName+". Do you want to continue?",
            function(){
                var uuu = "deleteDictionary/fw_dictionary/remove_AddressGroup";
