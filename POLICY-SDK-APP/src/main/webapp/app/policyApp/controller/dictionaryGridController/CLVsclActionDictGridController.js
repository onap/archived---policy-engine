...
app.controller('vsclActionDictGridController', function ($scope, PolicyAppService, modalService, $modal){
    ...
    PolicyAppService.getData('get_LockDownData').then(function(data){
		 ...
		 if($scope.lockdowndata[0].lockdown == true){
			 $scope.vsclActionDictionaryGrid.columnDefs[0].visible = false;
			 $scope.gridApi.grid.refresh();
		 }else{
			 $scope.vsclActionDictionaryGrid.columnDefs[0].visible = true;
			 $scope.gridApi.grid.refresh();
		 }
	 },function(error){
		 console.log("failed");
    ...
    $scope.vsclActionDictionaryGrid = {
        ...
        columnDefs: [{
            ...
            { field: 'vsclaction', displayName : 'VSCL Action', sort: { direction: 'asc', priority: 0 }},
            { field: 'description'},
    		...
    		success : function(resultList){
    			...
    			if($scope.success){
    				modalService.popupConfirmWin("Confirm","You are about to delete the Vscl Action Item  "+data.vsclaction+". \n "+searchString+" \n Do you want to continue?",
    						function(){
    					var uuu = "deleteDictionary/cl_dictionary/remove_VsclAction";
