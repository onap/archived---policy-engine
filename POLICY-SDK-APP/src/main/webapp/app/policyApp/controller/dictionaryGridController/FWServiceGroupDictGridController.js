 ...
 * ============LICENSE_END=========================================================
 */
app.controller('serviceGroupDictGridController', function ($scope, PolicyAppService, modalService, $modal){
    $( "#dialog" ).hide();
		
    PolicyAppService.getData('getDictionary/get_ServiceGroupData').then(function (data) {
    	var j = data;
	...
	PolicyAppService.getData('get_LockDownData').then(function(data){
		 ...
		 if($scope.lockdowndata[0].lockdown == true){
			 $scope.serviceGroupDictionaryGrid.columnDefs[0].visible = false;
			 $scope.gridApi.grid.refresh();
		 }else{
			 $scope.serviceGroupDictionaryGrid.columnDefs[0].visible = true;
			 $scope.gridApi.grid.refresh();
		 }
	 },function(error){
		 console.log("failed");
    ...
    $scope.serviceGroupDictionaryGrid = {
        ...
        },{ field: 'groupName', displayName : 'Group Name', sort: { direction: 'asc', priority: 0 }},
    ...
    $scope.deleteFWServiceGroup = function(data) {
        modalService.popupConfirmWin("Confirm","You are about to delete the Service Group:  "+data.groupName+". Do you want to continue?",
            function(){
                var uuu = "deleteDictionary/fw_dictionary/remove_serviceGroup";
