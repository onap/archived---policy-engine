 ...
 * ============LICENSE_END=========================================================
 */
app.controller('msLocationDictGridController', function ($scope, PolicyAppService, modalService, $modal){
    $( "#dialog" ).hide();
		
    PolicyAppService.getData('getDictionary/get_MicroServiceLocationData').then(function (data) {
    	var j = data;
    ...
    PolicyAppService.getData('get_LockDownData').then(function(data){
    	...
    	if($scope.lockdowndata[0].lockdown == true){
    		$scope.msLocationDictionaryGrid.columnDefs[0].visible = false;
    		$scope.gridApi.grid.refresh();
    	}else{
    		$scope.msLocationDictionaryGrid.columnDefs[0].visible = true;
    		$scope.gridApi.grid.refresh();
    	}
    },function(error){
    	console.log("failed");
    ...
    $scope.deleteMSLocation = function(data) {
        modalService.popupConfirmWin("Confirm","You are about to delete the Micro Service Location  "+data.name+". Do you want to continue?",
            function(){
                var uuu = "deleteDictionary/ms_dictionary/remove_msLocation";
