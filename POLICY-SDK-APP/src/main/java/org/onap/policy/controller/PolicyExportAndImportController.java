...
public class PolicyExportAndImportController extends RestrictedBaseController {
        ...
        for (Object object : entityData) {
            ...
            if (policyEntity.getPolicyName().contains(DECISION_MS) || !policyEntity.getPolicyName().contains("Decision_")) {
                if (policyEntity.getConfigurationData() != null) {
                    ...
                    row = populateConfigParam(policyEntity, row, sheet, headingRow, body);//
                }
                row = populateActionBodyEntity(policyEntity, row);//
            } else {
    ...
    private HSSFRow populateConfigParam(PolicyEntity policyEntity, HSSFRow row, 
            HSSFSheet sheet, HSSFRow headingRow, String body) {
        ...
        try (FileInputStream excelFile = new FileInputStream(new File(file));
            ...
            while (rowIterator.hasNext()) {
                ...
                while (cellIterator.hasNext()) {
                    ...
                    configurationBodySet = isConfigurationBodySet(policyEntity, cell, setBodySize, setBodySize, configurationBodySet);//
                    ...
                    actionBodyEntity = setActionBodyObject(policyEntity, actionBodyEntity, cell, actionExists);//
                    bodySize = getBobySize(bodySize, cell); //
                    configName = getConfigName(policyEntity, cell, configName);//
                    configurationDataEntity =
                            populateConfigurationDataEntity(policyEntity, configurationDataEntity, cell); //
                    actionBodyEntity = populateActionBodyObject(policyEntity, actionBodyEntity, cell, actionExists);//
        ...
        try {
            ...
            if (jsonObject != null) {
                ...
                if (!isAttributeObjectFound(msService, msVersion)) {
                    return POLICY + policyName + " MS Service: " + msService + " and MS Version: " + msVersion
                            + NOTVALID;
                }
            }
        ...
        if (microServiceModelsData != null) {
            for (int i = 0; i < microServiceModelsData.size(); i++) {
                ...
                if (workingModel != null) {
                    if (workingModel.getVersion() != null && workingModel.getVersion().equals(msVersion)) {
                        return true;
                    }
                }
            }
    ...
    private boolean isContinue(List<Object> queryData, List<String> roles, String scope, UserInfo userInfo,
        ...
        if (isContinue(roles, scope, userInfo, scopes)) {
            return true;
        }
        return false;
    }
    ...
    private boolean isFinalColumn(PolicyEntity policyEntity, Cell cell, int setBodySize, int bodySize, boolean finalColumn) {
        if (configurationbody.equalsIgnoreCase(getCellHeaderName(cell))) {
            if (policyEntity.getPolicyName().contains(config) ||
                    policyEntity.getPolicyName().contains(DECISION_MS)) {
    ...
    private boolean isConfigurationBodySet(PolicyEntity policyEntity, Cell cell, int setBodySize, int bodySize, boolean configurationBodySet) {
        if (configurationbody.equalsIgnoreCase(getCellHeaderName(cell))) {
            if (policyEntity.getPolicyName().contains(config) ||
                    policyEntity.getPolicyName().contains(DECISION_MS)) {
                if (setBodySize == bodySize) {
    ...
    private boolean isActionExists(PolicyEntity policyEntity, Cell cell, boolean actionExists) {
        if (configurationbody.equalsIgnoreCase(getCellHeaderName(cell))) {
            if (policyEntity.getPolicyName().contains("Action_")) {
                actionExists = true;
            }
        }
        return actionExists;
    ...
    private ActionBodyEntity setActionBodyObject(PolicyEntity policyEntity, ActionBodyEntity actionBodyEntity,
            Cell cell, boolean actionExists) {        
        if (configurationbody.equalsIgnoreCase(getCellHeaderName(cell))) {
            if(policyEntity.getPolicyName().contains("Action_")){
    ...
    private String getConfigName(PolicyEntity policyEntity, Cell cell, String configName) {
        if (configurationName.equalsIgnoreCase(getCellHeaderName(cell))) {
