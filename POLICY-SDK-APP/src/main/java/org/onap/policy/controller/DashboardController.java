...
public class DashboardController extends RestrictedBaseController {
	private static final Logger policyLogger = FlexLogger
	...
	private static final String FIRE_COUNT = "fireCount";
	private static final String JMX_CONNCTION = " for JMX connection";
	...
	private PolicyController getPolicyControllerInstance() {
		return policyController != null ? getPolicyController()
	...
	public void getData(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			model.put("availableLoggingDatas",
					mapper.writeValueAsString(systemDAO.getLoggingData()));
			JsonMessage msg = new JsonMessage(mapper.writeValueAsString(model));
			JSONObject j = new JSONObject(msg);
			response.getWriter().write(j.toString());
		} catch (Exception e) {
			policyLogger.error(EXCEPTION, e);
		}
	}

	@RequestMapping(value = { "/get_DashboardPolicyCRUDData" }, method = { org.springframework.web.bind.annotation.RequestMethod.GET }, produces = MediaType.APPLICATION_JSON_VALUE)
	public void get_DashboardPolicyCRUDData(HttpServletRequest request,
		...
		try {
			...
			model.put("papStatusCRUDData",
					mapper.writeValueAsString(policyStatusCRUDData));
			JsonMessage msg = new JsonMessage(mapper.writeValueAsString(model));
			JSONObject j = new JSONObject(msg);
			response.getWriter().write(j.toString());
		} catch (Exception e) {
	...
	@RequestMapping(value = { "/dashboardController/dashboardAdvancedSearch.htm" }, method = { org.springframework.web.bind.annotation.RequestMethod.POST })
	public ModelAndView dashboardAdvancedSearch(HttpServletRequest request,
			HttpServletResponse response) throws IOException {
		String scope = null;
		String stage = "both";
		String isDelected = "both";
		String ttlDate_after = "";
		String ttlDate_before = "";
		try {
			ObjectMapper mapper = new ObjectMapper();
			mapper.setVisibility(PropertyAccessor.FIELD, Visibility.ANY);
			...
			if (jscope != null) {
				scope = jscope.textValue();
			}
			JsonNode jstage = inputData.path(STAGE);
			if (jstage != null) {
				stage = jstage.textValue();
			}
			JsonNode jisDelected = inputData.path("isDelected");
			if (jisDelected != null) {
				isDelected = jisDelected.textValue();
			}
			JsonNode jttlDate_after = inputData.path("ttlDate_after");
			if (jttlDate_after != null) {
				ttlDate_after = jttlDate_after.textValue();
			}
			JsonNode jttlDate_before = inputData.path("ttlDate_before");
			if (jttlDate_before != null) {
				ttlDate_before = jttlDate_before.textValue();
			}
			List<Object> policyData = queryPolicyData(scope, stage, isDelected,
					ttlDate_after, ttlDate_before);
			populatePolicyData(policyData, scope, stage);
			Map<String, Object> model = new HashMap<>();
			model.put("policyStatusCRUDData",
					mapper.writeValueAsString(policyStatusCRUDData));
			JsonMessage msg = new JsonMessage(mapper.writeValueAsString(model));
			JSONObject j = new JSONObject(msg);
			response.getWriter().write(j.toString());
	...
	@RequestMapping(value = { "/get_DashboardPdpPolicyCRUDData" }, method = { org.springframework.web.bind.annotation.RequestMethod.GET }, produces = MediaType.APPLICATION_JSON_VALUE)
	public void get_DashboardPdpPolicyCRUDData(HttpServletRequest request,
			HttpServletResponse response) {
		try {
			Map<String, Object> model = new HashMap<>();
			ObjectMapper mapper = new ObjectMapper();
			mapper.setVisibility(PropertyAccessor.FIELD, Visibility.ANY);
			model.put("pdpStatusCRUDData",
					mapper.writeValueAsString(pdpStatusCRUDData));
			JsonMessage msg = new JsonMessage(mapper.writeValueAsString(model));
			JSONObject j = new JSONObject(msg);
			response.getWriter().write(j.toString());
		} catch (Exception e) {
	...
	public void getSystemAlertData(HttpServletRequest request,
			HttpServletResponse response) {
		try {
			Map<String, Object> model = new HashMap<>();
			ObjectMapper mapper = new ObjectMapper();
			model.put("systemAlertsTableDatas",
					mapper.writeValueAsString(systemDAO.getSystemAlertData()));
			JsonMessage msg = new JsonMessage(mapper.writeValueAsString(model));
			JSONObject j = new JSONObject(msg);
			response.getWriter().write(j.toString());
		} catch (Exception e) {
	...
	public void getPAPStatusData(HttpServletRequest request,
			HttpServletResponse response) {
		try {
			Map<String, Object> model = new HashMap<>();
			ObjectMapper mapper = new ObjectMapper();
	...
	public void getPDPStatusData(HttpServletRequest request,
			HttpServletResponse response) {
		try {
			Map<String, Object> model = new HashMap<>();
			ObjectMapper mapper = new ObjectMapper();
	...
	public void getPolicyActivityData(HttpServletRequest request,
			HttpServletResponse response) {
		try {
			Map<String, Object> model = new HashMap<>();
			ObjectMapper mapper = new ObjectMapper();
			...
			this.pdpConatiner = new PDPGroupContainer(controller.getPapEngine());
			addPolicyToTable();
			model.put("policyActivityTableDatas",
					mapper.writeValueAsString(policyActivityData));
			JsonMessage msg = new JsonMessage(mapper.writeValueAsString(model));
			JSONObject j = new JSONObject(msg);
			response.getWriter().write(j.toString());
		} catch (Exception e) {
	...
	public void addPAPToTable() {
		...
		try {
			...
			policyLogger.error(
					"Error getting PAP status, PAP not responding to requests",
					e1);
		}
		String papURL = XACMLProperties
				.getProperty(XACMLRestProperties.PROP_PAP_URL);
	...
	public void addPolicyCRUDInfoToTable() {
		...
		geParams.put(CREATED_DATE, lastThirtyDate.toString());
		List<Object> datas = commonClassDao.getDataByQuery(entityquery,
				geParams);
		if (datas != null) {
			datas.stream()
					.forEach(
							(data) -> {
								JSONObject object = new JSONObject();
								object.put("id",
										((PolicyEntity) data).getPolicyId());
								object.put(SCOPE,
										((PolicyEntity) data).getScope());
								object.put("policyName",
										((PolicyEntity) data).getPolicyName());
								object.put("version",
										((PolicyEntity) data).getVersion());
								if (isPushedToPDP(((PolicyEntity) data)
										.getPolicyId())) {
								...
								object.put("createdBy",
										((PolicyEntity) data).getCreatedBy());
								object.put("deleted",
										((PolicyEntity) data).isDeleted());
								object.put("deleteReasonCode",
										((PolicyEntity) data)
												.getDeleteReasonCode());
								object.put("deletedBy",
										((PolicyEntity) data).getDeletedBy());
								object.put("modifiedBy",
										((PolicyEntity) data).getModifiedBy());
								if (((PolicyEntity) data).getModifiedDate() != null) {
									Timestamp ts = new Timestamp(
											((PolicyEntity) data)
													.getModifiedDate()
													.getTime());
									Timestamp cts = new Timestamp(
											((PolicyEntity) data)
													.getCreatedDate().getTime());
									object.put(MODIFIED_DATE, ts.toString());
									object.put(CREATED_DATE, cts.toString());
								} else {
	...
	private boolean isPushedToPDP(long policyId) {
		try {
			...
			List<Object> groupobject = commonClassDao.getDataByQuery(
					groupEntityquery, geParams);
	 ...
	 */
	private List<Object> queryPolicyData(String scope, String stage,
			String isDeleted, String createdAfter, String createdBefore) {
		policyStatusCRUDData = new ArrayList<>();
		...
		try {
			...
			StringBuilder entityquery = new StringBuilder(
					"from PolicyEntity where scope like :scope ");
			...
			policyData = commonClassDao.getDataByQuery(entityquery.toString(),
					geParams);
			if (policyData != null) {
				policyLogger.info("getPolicyData : " + policyData.size());
			} else {
				policyLogger.info("getPolicyData : no data found.");
			}
		} catch (Exception e) {
			policyLogger.error(EXCEPTION, e);
		}
	...
	private void populatePolicyData(List<Object> policyData, String scope,
			String stage) {
		if (policyData != null) {
			policyData
					.stream()
					.forEach(
							(data) -> {
								...
								if ("PDP".equalsIgnoreCase(stage) && isPDP) {
									policyStatusCRUDData.add(object);
								} else if ("PAP".equalsIgnoreCase(stage)
										&& !isPDP) {
									policyStatusCRUDData.add(object);
								} else if ("both".equalsIgnoreCase(stage)) {
									policyStatusCRUDData.add(object);
								}
	...
	public void addPDPToTable() {
		...
		for (PDPGroup group : this.pdpConatiner.getGroups()) {
			for (PDP pdp : group.getPdps()) {
				naCount = -1;
				if ("UP_TO_DATE".equals(pdp.getStatus().getStatus().toString())
						&& ((OnapPDP) pdp).getJmxPort() != 0) {
					String pdpIpAddress = parseIPSystem(pdp.getId());
					int port = ((OnapPDP) pdp).getJmxPort();
					if (port != 0) {
						policyLogger.debug("Getting JMX Response Counts from "
								+ pdpIpAddress + " at JMX port " + port);
						naCount = getRequestCounts(pdpIpAddress, port,
								"pdpEvaluationNA");
						permitCount = getRequestCounts(pdpIpAddress, port,
								"PdpEvaluationPermit");
						denyCount = getRequestCounts(pdpIpAddress, port,
								"PdpEvaluationDeny");
					}
				}
				if (naCount == -1) {
	...
	/*
	 * Contact JMX Connector Sever and return the value of the given
	 * jmxAttribute
	 */
	...
	private long getRequestCounts(String host, int port, String jmxAttribute) {
		...
		policyLogger
				.debug("Create an RMI connector client and connect it to the JMX connector server");
		HashMap map = null;
		try (JMXConnector jmxConnection = JMXConnectorFactory.newJMXConnector(
			...
			policyLogger.error("Error in reteriving" + jmxAttribute
					+ " from JMX connection", e);
		} catch (AttributeNotFoundException e) {
			policyLogger.error("AttributeNotFoundException  " + jmxAttribute
					+ JMX_CONNCTION, e);
		} catch (InstanceNotFoundException e) {
			policyLogger.error("InstanceNotFoundException " + host
					+ JMX_CONNCTION, e);
		} catch (MalformedObjectNameException e) {
			policyLogger.error(
					"MalformedObjectNameException for JMX connection", e);
	...
	private void addPolicyToTable() {
		...
		for (PDPGroup group : this.pdpConatiner.getGroups()) {
			for (PDPPolicy policy : group.getPolicies()) {
				try {
					policyMap.put(policy.getPolicyId().replace(" ", ""),
							policy.getId());
				} catch (Exception e) {
					policyLogger.error(XACMLErrorConstants.ERROR_SCHEMA_INVALID
							+ policy.getName() + e);
				}
			}
			...
			for (PDP pdp : group.getPdps()) {
				...
				if ("UP_TO_DATE".equals(pdp.getStatus().getStatus().toString())
						&& ((OnapPDP) pdp).getJmxPort() != 0) {
					String pdpIpAddress = parseIPSystem(pdp.getId());
					policyList = getPolicy(pdpIpAddress,
							((OnapPDP) pdp).getJmxPort(), "policyCount");
				}
				if (policyList != null && policyList.toString().length() > 3) {
					...
					for (String policyKeyValue : splitPolicy) {
						...
						if (policyID != null && policyMap.containsKey(policyID)) {
							JSONObject object = new JSONObject();
							object.put(POLICY_ID, policyMap.get(policyID));
							object.put(FIRE_COUNT, policyFireCount);
					...
					if (policyList != null) {
						JSONObject object = new JSONObject();
						object.put(POLICY_ID,
								"Unable to retrieve policy information");
						object.put(FIRE_COUNT, "NA");
						object.put(SYSTEM, pdp.getId());
						policyActivityData.add(object);
					} else {
						JSONObject object = new JSONObject();
						object.put(POLICY_ID, "Unable to access PDP JMX Server");
						object.put(FIRE_COUNT, "NA");
						object.put(SYSTEM, pdp.getId());
						policyActivityData.add(object);
					}
	...
	private Object getPolicy(String host, int port, String jmxAttribute) {
		policyLogger
				.debug("Create an RMI connector client and connect it to the JMX connector server for Policy: "
						+ host);
		HashMap map = null;
		try (JMXConnector jmxConnection = JMXConnectorFactory.newJMXConnector(
				createConnectionURL(host, port), map)) {
			jmxConnection.connect();
			Object o = jmxConnection.getMBeanServerConnection().getAttribute(
