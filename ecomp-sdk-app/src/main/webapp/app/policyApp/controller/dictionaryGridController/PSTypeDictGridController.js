...
/**
 */
app.controller('psTypeDictGridController', function ($scope, PolicyScopeService,modalService, $modal, PapUrlService){
    $( "#dialog" ).hide();
	...
	PapUrlService.getPapUrl().then(function(data) {
	    ...
	    PolicyScopeService.getPSTypeDictionaryData(papUrl).then(function (data) {
	        var j = data;
    ...
    $scope.deletePSType = function(data) {
        modalService.popupConfirmWin("Confirm","You are about to delete the Type  "+data.name+". Do you want to continue?",
            function(){
                var uuu = papUrl + "/ecomp/ps_dictionary/remove_PSType.htm";
