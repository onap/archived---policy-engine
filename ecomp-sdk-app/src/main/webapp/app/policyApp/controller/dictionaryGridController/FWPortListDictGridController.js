...
/**
 */
app.controller('portListDictGridController', function ($scope, FWDictionaryService,modalService, $modal, PapUrlService){
    $( "#dialog" ).hide();
	...
	PapUrlService.getPapUrl().then(function(data) {
		...
		FWDictionaryService.getPortListDictionaryData(papUrl).then(function (data) {
	        var j = data;
    ...
    $scope.portListDictionaryGrid = {
        ...
        },{ field: 'portName', displayName : 'Port Name'},
            { field: 'description' }
        ]
    };
    ...
    $scope.deleteFWPortList = function(data) {
        modalService.popupConfirmWin("Confirm","You are about to delete the Port List  "+data.portName+". Do you want to continue?",
            function(){
                var uuu = papUrl + "/ecomp/fw_dictionary/remove_PortList.htm";
