...
/**
 */
app.controller('decisionSettingsDictGridController', function ($scope, DictionaryService,modalService, $modal, PapUrlService){
    $( "#dialog" ).hide();
	var papUrl;
	PapUrlService.getPapUrl().then(function(data) {
		...
		DictionaryService.getDecisionSettingsDictionaryData(papUrl).then(function (data) {
	        var j = data;
    ...
    $scope.decisionSettingsDictionaryGrid = {
        ...
        },{ field: 'xacmlId', displayName : 'Settings ID'},
    ...
    $scope.deleteSettingsDict = function(data) {
        modalService.popupConfirmWin("Confirm","You are about to delete the Settings Dictionary  "+data.xacmlId+". Do you want to continue?",
            function(){
                var uuu = papUrl + "/ecomp/settings_dictionary/remove_settings.htm";
