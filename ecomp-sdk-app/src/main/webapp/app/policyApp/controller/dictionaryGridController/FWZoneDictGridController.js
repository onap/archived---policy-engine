...
/**
 */
app.controller('zoneDictGridController', function ($scope, FWDictionaryService,modalService, $modal, PapUrlService){
    $( "#dialog" ).hide();
	...
	PapUrlService.getPapUrl().then(function(data) {
		...
		FWDictionaryService.getZoneDictionaryData(papUrl).then(function (data) {
	        var j = data;
    ...
    $scope.deleteFWZone = function(data) {
        modalService.popupConfirmWin("Confirm","You are about to delete the Zone Name  "+data.zoneName+". Do you want to continue?",
            function(){
                var uuu = papUrl + "/ecomp/fw_dictionary/remove_zone.htm";
