...
public abstract class Policy {
	...
	/**
	 * Return the data field of the PolicyAdapter that will be used when saving
	 * this policy with the savePolicies method.
	 * 
	 * @return Either the PolicyAdapter.getData() or
		...
		if (policyData instanceof PolicyType || policyData instanceof PolicySetType) {
			...
			try {
				if (policyData instanceof PolicyType) {
					...
					inputStream = XACMLPolicyWriter.getPolicySetXmlAsInputStream((PolicySetType) policyData);
				}
	...
	/**
	 * @param preparedToSave
	 *            the preparedToSave to set
	 */
