...
public class ActionPolicyTest {
	...
	public void setUp() throws Exception {
		...
		policyAdapter.setPolicyName("Mike.Action_junitTest.1.xml");
	...
	public void testSavePolicies() {
        ...
        Map<String, String> oldsuccessMap = new HashMap<>();
        
		successMap.put("success", "success");
        when(mockPolicy.createPolicy(Paths.get(policyAdapter.getNewFileName()), policyAdapter.getData())).thenReturn(successMap);

        try {
        	when(mockAction.savePolicies()).thenReturn(successMap);
        	successMap = mockAction.savePolicies();
			oldsuccessMap = component.savePolicies();
		} catch (Exception e) {
			e.printStackTrace();
		}
		assertEquals(successMap.get("success"),"success");
		assertEquals(oldsuccessMap.get("error"),"Validation Failed");
